...
class Camera:
	...
	def update(self, w, h):
		screen = Point(w, h) / self.game_map.chunk
		...
		chunks_draw = (self.new_grid.trunc() - self.new_grid) * self.game_map.chunk 		# Переменная для отрисовки линий чанков

		self.lines = []

		# Отрисовка линий чанков по X
		for i in range(chunks.x + 2):
			new_x = chunks_draw.x + (self.game_map.chunk.x * i)
			self.lines.append([[new_x, 0], [new_x, h]])

		# Отрисовка линий чанков по Y
		for i in range(chunks.y + 2):
			new_y = chunks_draw.y + (self.game_map.chunk.y * i)
	...
	def draw(self, game):
		...
		for obj in self.game_map.objects:
			if (obj.glob > self.invert_new_grid) and (obj.glob < self.new_grid):
				new_coord = (obj.glob - self.new_grid) * self.game_map.chunk
